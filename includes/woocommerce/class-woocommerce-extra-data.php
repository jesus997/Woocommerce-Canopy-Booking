...
class WCB_Woocommerce_Extra_Data {
    ...
    function tours_add_item_data($cart_item_data, $product_id, $variation_id) {
        ...
        if(!$discount_active) {
            ...
            $discount_active = !$valid_plus_promotion;
        } else {
            if(!$souvenir_active) {
                $cart_item_dasta["__is_plus"] = false;
            }
        }
    ...
    function is_valid_plus_promotion($product_id, $rdate = false) {
        if(!$rdate) $rdate = date("d/m/Y");
        $souvenir_days_to_apply = value("days_of_anticipation_to_apply_this_souvenir", 0, $product_id);
        return $souvenir_days_to_apply > 0 ? boolval($this->calculate_date_discount($rdate, $souvenir_days_to_apply, 1, "<=")) : true;
    }
        ...
        if ( ! empty( $cart->cart_contents ) ) {
            foreach ( $cart->cart_contents as $cart_item_key => $cart_item ) {
                ...
                if($product->is_type( 'canopytour' ) || get_post_type($product->get_id()) === "product_variation") {
                    ...
                    $souvenir_is_valid = $this->is_valid_plus_promotion($product->get_id(), $rdate);
                    ...
                    if($souvenir_active) {
                        if(!$discount_active) {
                            $discount_active = !$souvenir_is_valid;
                        }
