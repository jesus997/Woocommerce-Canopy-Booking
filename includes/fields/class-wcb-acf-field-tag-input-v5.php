...
if( !class_exists('wcb_acf_field_tag_input') ) :


class wcb_acf_field_tag_input extends acf_field {
	...
	*/
	...
	function __construct( $settings ) {
		...
		*/
		...
		$this->l10n = array(
			'error'	=> __('Error! Please enter a higher value', 'wcb'),
		);
	...
	*/
	...
	function render_field_settings( $field ) {
        ...
        */
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Duplicate Tag Class','wcb'),
			'instructions'	=> __('Customise the input class','wcb'),
			'type'			=> 'text',
			'name'			=> 'duplicateTagClass',
		));

        acf_render_field_setting( $field, array(
			'label'			=> __('Allowed Tags','wcb'),
			'instructions'	=> __('','wcb'),
            'type'			=> 'text',
			'name'			=> 'allowedTags'
        ));
        ...
        acf_render_field_setting( $field, array(
			'label'			=> __('Regular Expression','wcb'),
			'instructions'	=> __('','wcb'),
            'type'			=> 'text',
			'name'			=> 'regex'
		));
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		// register & include JS
		wp_register_script('wcb-acf-plugin', "{$url}fields/js/taggle.min.js", array('acf-input'), $version);
		wp_enqueue_script('wcb-acf-plugin');
		
		
		// register & include CSS
		wp_register_style('wcb-acf-plugin', "{$url}fields/css/taggle.css", array('acf-input'), $version);
		wp_enqueue_style('wcb-acf-plugin');
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( $value < $field['custom_minimum_setting'] )
		{
			$valid = __('The value is too little!','wcb'),
		}
...
// initialize
new wcb_acf_field_tag_input( $this->settings );
